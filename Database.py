...
def DatabaseCreate():
    cnx = mysql.connector.connect(user='root', password='Niet@567', host='localhost')
    Cursor = cnx.cursor()
...
def TablesCreate():
    cnx = mysql.connector.connect(user='root', password='Niet@567', host='localhost', database='Library')
    Cursor = cnx.cursor()
